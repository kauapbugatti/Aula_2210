...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $emailErro = null;
    $idadeErro = null;
    $telefoneErro = null; // Nova variável de erro para telefone
    $validacao = true;
    ...
    if (!empty($_POST)) {
        ...
        if (!empty($_POST['telefone'])) {
            ...
            if (!preg_match('/^\(\d{2}\) \d{5}-\d{4}$/', $telefone)) {
                $telefoneErro = 'Por favor digite um telefone válido! (ex: (99) 99999-9999)';
                $validacao = false;
            }
        } else {
    ...
    if ($validacao) {
        ...
        // Atualize a query para incluir telefone
        $sql = "INSERT INTO tb_alunos (nome, endereco, email, idade, telefone) VALUES (?, ?, ?, ?, ?)";
        $q = $pdo->prepare($sql);
       
        try {
            // Adicione o telefone na execução
            $q->execute(array($nome, $endereco, $email, $idade, $telefone));
            Banco::desconectar();
            ...
            <div class="card-body">
                <form class="form-horizontal" action="create.php" method="post">
                    ...
                    <div class="control-group <?php echo !empty($telefoneErro) ? 'error ' : ''; ?>">
                        ...
                        <div class="controls">
                            <input size="15" class="form-control" name="telefone" type="text" placeholder="(99) 99999-9999" value="<?php echo !empty($telefone) ? $telefone : ''; ?>">
